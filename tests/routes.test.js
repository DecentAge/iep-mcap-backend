...
describe('GET /api/v1/get', () => {
    it('returns empty array when no currencies exist', async () => {
        const res = await request(app).get(base + '/api/v1/get');
        expect(res.status).toBe(200);
    ...
    it('respects the results limit parameter', async () => {
        ...
        const res = await request(app).get(base + '/api/v1/get?results=2&page=1');
        expect(res.status).toBe(200);
    ...
    it('returns second page when page=2', async () => {
        ...
        const res = await request(app).get(base + '/api/v1/get?results=2&page=2');
        expect(res.status).toBe(200);
        expect(res.body.length).toBe(2);
    ...
    it('finds a currency by name (id lookup)', async () => {
        ...
        const res = await request(app).get(base + '/api/v1/get?name=Bitcoin');
        expect(res.status).toBe(200);
    ...
    it('24h filter sorts by percent_change_24h desc', async () => {
        ...
        const res = await request(app).get(base + '/api/v1/get?filter=24h');
        expect(res.status).toBe(200);
    ...
    it('7d filter sorts by percent_change_7d desc', async () => {
        ...
        const res = await request(app).get(base + '/api/v1/get?filter=7d');
        expect(res.status).toBe(200);
    ...
    it('order=desc reverses default sort', async () => {
        ...
        const res = await request(app).get(base + '/api/v1/get?order=desc');
        expect(res.status).toBe(200);
...
describe('admin endpoints', () => {
    ...
    it('rejects /api/v1/fetch without key', async () => {
        const res = await request(app).get(base + '/api/v1/fetch');
        expect(res.status).toBe(400);
        expect(res.body.success).toBe(false);
    });

    it('rejects /api/v1/fetch with wrong key', async () => {
        const res = await request(app).get(base + '/api/v1/fetch?key=wrong');
        expect(res.status).toBe(400);
        expect(res.body.success).toBe(false);
    });

    it('rejects /api/v1/fetch/deactivate without key', async () => {
        const res = await request(app).get(base + '/api/v1/fetch/deactivate');
        expect(res.status).toBe(400);
        expect(res.body.success).toBe(false);
    });

    it('rejects /api/v1/fetch/activate without key', async () => {
        const res = await request(app).get(base + '/api/v1/fetch/activate');
        expect(res.status).toBe(400);
        expect(res.body.success).toBe(false);
    });
});
